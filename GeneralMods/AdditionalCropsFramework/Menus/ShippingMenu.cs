...
namespace AdditionalCropsFramework.Menus
{
    class ShippingMenuExpanded : StardewValley.Menus.ShippingMenu
    {
        ...
        public ShippingMenuExpanded(List<Item> items) : base(items)
        {
        ...
        public new void parseItems(List<Item> items)
        {
            Log.AsyncY("PARSE IT!!!");
            ...
            foreach (Item obj in items)
            {
                ...
                if (obj is StardewValley.Object)
                {
                    ...
                    if ((obj as StardewValley.Object).Category == -75 || (obj as StardewValley.Object).Category == -79)
                        Game1.stats.CropsShipped += (uint)(obj as StardewValley.Object).Stack;
                    if ((obj as StardewValley.Object).countsForShippedCollection())
                        Game1.player.shippedBasic((obj as StardewValley.Object).parentSheetIndex, (obj as StardewValley.Object).stack);
                }
                else if (obj is StardustCore.CoreObject)
                {
                    Log.AsyncG("WTFFFFFFF PRICE :");

                    int categoryIndexForObject = this.getCategoryIndexForObject((obj as StardustCore.CoreObject));
                    this.categoryItems[categoryIndexForObject].Add((Item)(obj as StardustCore.CoreObject));
                    List<int> categoryTotals = this.categoryTotals;
                    int index = categoryIndexForObject;
                    categoryTotals[index] = categoryTotals[index] + StardustCore.Utilities.sellToStorePrice((obj as StardustCore.CoreObject)) * (obj as StardustCore.CoreObject).Stack;
                    Game1.stats.itemsShipped += (uint)(obj as StardustCore.CoreObject).Stack;
                    if ((obj as StardustCore.CoreObject).Category == -75 || (obj as StardewValley.Object).Category == -79)
                        Game1.stats.CropsShipped += (uint)(obj as StardustCore.CoreObject).Stack;
                    if ((obj as StardustCore.CoreObject).countsForShippedCollection())
                        Game1.player.shippedBasic((obj as StardustCore.CoreObject).parentSheetIndex, (obj as StardustCore.CoreObject).stack);
                }
                else if(obj is AdditionalCropsFramework.ModularCropObject)
                {
                    Log.AsyncG("PRICES BISCUITS!!!");
                    categoryTotals[5] += obj.salePrice();

                }
                else if(obj is Item)
                {
                    Log.AsyncG("PRICES PUPPERS!!!");
                    categoryTotals[5] += obj.salePrice();
                }
            }
        ...
        public override void draw(SpriteBatch b)
        {
           ...
           // Log.AsyncC("PAGE "+this.currentPage);
            if (this.currentPage == -1)
            {
                SpriteText.drawStringWithScrollCenteredAt(b, Utility.getDateString(0), Game1.viewport.Width / 2, this.categories[0].bounds.Y - Game1.tileSize * 2, "", 1f, -1, 0, 0.88f, false);
            ...
            if (this.outro)
            {
                ...
                SpriteText.drawStringWithScrollCenteredAt(b, this.newDayPlaque ? Utility.getDateString(1) : Utility.getDateString(0), Game1.viewport.Width / 2, this.dayPlaqueY, "", 1f, -1, 0, 0.88f, false);
                foreach (TemporaryAnimatedSprite animation in this.animations)
                    animation.draw(b, true, 0, 0);
                if (this.finalOutroTimer > 0)
